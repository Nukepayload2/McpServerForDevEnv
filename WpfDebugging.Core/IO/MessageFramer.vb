...
Public NotInheritable Class MessageFramer
    ...
    Public Shared Async Function WriteAsync(stream As Stream, payload As Object, Optional cancellationToken As CancellationToken = Nothing) As Task
        ...
        ' 4 字节大端长度：每个字节先 And &HFF 取低 8 位再转 Byte，
        ' 避免 body.Length 较大时移位/转换结果超 Byte 范围抛 OverflowException。
        header(0) = CByte((body.Length >> 24) And &HFF)
        header(1) = CByte((body.Length >> 16) And &HFF)
        header(2) = CByte((body.Length >> 8) And &HFF)
        header(3) = CByte(body.Length And &HFF)
    ...
    Public Shared Async Function ReadAsync(Of T As Class)(stream As Stream, Optional cancellationToken As CancellationToken = Nothing) As Task(Of T)
        ...
        ' 4 字节大端长度解码：先把 Byte 转 Integer 再移位/或运算。
        ' VB 的 << 对 Byte 左操作数返回 Byte，header(2)<<8 会溢出 Byte 范围（如 137<<8=35072 截断），
        ' 导致大响应（>255 字节）的 length 解码错误、JSON 被截断、反序列化失败。
        Dim length As Integer = (CInt(header(0)) << 24) Or (CInt(header(1)) << 16) Or (CInt(header(2)) << 8) Or CInt(header(3))
